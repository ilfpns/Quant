...
`torch.quantization.prepare_qat`으로 fake-quant를 삽입한 뒤, FP32 모델을 teacher로 삼아 (임의 입력 → FP32 출력) 쌍으로 self-distillation 학습을 진행하고 `convert`로 변환함(`quanti/qat.py`의 `qat_quantize`, `build_self_distillation_data`).
